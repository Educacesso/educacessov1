...
namespace educacesso
{
    public class CursoAddDAO
    {
        ...
        {//FALTA IMPLEMENTAR A RESPOSTA CERTA
            ...
            cmd.Parameters.AddWithValue("@COD", cod_curso);
			...
			cmd.Parameters.AddWithValue("@RESPOSTA_CERTA", resposta_certa);
        ...
        {
            SqlCommand cmd = new SqlCommand("DELETE tblLicao FROM tblLicao INNER JOIN tblCurso ON tblLicao.COD_CURSO = tblCurso.COD_CURSO DELETE tblExercicio FROM tblExercicio INNER JOIN tblCurso ON tblExercicio.COD_CURSO = tblCurso.COD_CURSO WHERE tblCurso.COD_CURSO = @COD", new ConnectionFactory().getConnection());
